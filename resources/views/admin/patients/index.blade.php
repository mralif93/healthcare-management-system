...
<div class="space-y-6">
    ...
    <div class="bg-brand-600 rounded-2xl p-8 text-white shadow-xl shadow-brand-100 relative overflow-hidden animate__animated animate__fadeInUp animate__faster">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
                ...
                <p class="text-sm text-white/70 mt-1">Clinical records &amp; patient management</p>
            </div>
            <a href="{{ route('admin.patients.create') }}" class="px-5 py-2.5 bg-white/20 hover:bg-white/30 border border-white/30 backdrop-blur-sm text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center space-x-2 shrink-0">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('admin.patients.index') }}" method="GET" class="flex items-center gap-3">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('admin.patients.index') }}" class="text-xs font-semibold text-slate-400 hover:text-slate-600 transition-colors flex-shrink-0">× Clear</a>
                @endif
            </form>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group text-xs">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end space-x-2">
                                ...
                                <form id="del-patient-{{ $patient->id }}" action="{{ route('admin.patients.destroy', $patient) }}" method="POST" class="hidden">
                                    @csrf @method('DELETE')
                                </form>
                                <button type="button"
                                        onclick="openConfirmModal('delete', 'Delete Patient Record', 'Are you sure you want to permanently delete this patient record? This action cannot be undone.', 'del-patient-{{ $patient->id }}')"
                                        class="w-8 h-8 rounded-full bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-600 hover:text-white transition-all shadow-sm" title="Delete Permanent">
