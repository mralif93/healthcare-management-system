...
<head>
    ...
    <title>@yield('title', 'Admin') | {{ $clinicName }}</title>
...
<body class="bg-slate-100 font-sans antialiased text-slate-900 overflow-hidden">
        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 flex-shrink-0 flex flex-col
            ...
            <div class="h-16 flex items-center px-5 border-b border-slate-800 flex-shrink-0">
                <div class="flex items-center space-x-3">
                    <div
                        class="w-9 h-9 bg-brand-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-brand-900/50 flex-shrink-0">
                        <i class="hgi-stroke hgi-hospital-01 text-lg"></i>
                    </div>
                    <div>
                        <p class="text-sm font-bold text-white leading-none tracking-tight">{{ $clinicName }}</p>
                        <p class="text-[9px] font-semibold text-brand-400 uppercase tracking-widest mt-0.5">Admin Portal
                        </p>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 px-3 py-4 overflow-y-auto">
                @php
                $navItems = config('layout.navigation.admin');
                ...
                <div class="mb-4">
                    ...
                    <div class="space-y-0.5">
                        ...
                        $isSubItem = in_array($item['route'], ['admin.users.create', 'admin.patients.create',
        ...
        <main class="flex-1 flex flex-col overflow-hidden">
            ...
            <header
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 class="text-sm font-bold text-slate-900 leading-none">@yield('page_title', 'Overview')</h1>
                ...
                <div class="flex items-center space-x-2">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak
                            ...
                            <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                                <div class="flex items-center space-x-2">
                                    ...
                                    @if($unreadCount > 0)
                                    <span
                                        class="px-1.5 py-0.5 bg-brand-600 text-white rounded-full text-[9px] font-bold">{{
                                        $unreadCount }}</span>
                                    @endif
                                </div>
                                @if($unreadCount > 0)
                                <form action="{{ route('notifications.readAll') }}" method="POST">
                                    @csrf
                                    <button
                                        class="text-[10px] font-semibold text-brand-600 hover:text-brand-800 transition-colors">Mark
                                        all read</button>
                                </form>
                                @endif
                            </div>
                            <div class="max-h-72 overflow-y-auto divide-y divide-slate-50">
                                ...
                                <div class="flex items-start space-x-3 px-4 py-3 {{ $notif->read_at ? '' : 'bg-brand-50/60' }} hover:bg-slate-50 transition-colors cursor-pointer"
                                    ...
                                    <div
                                        class="w-8 h-8 rounded-xl {{ $notif->read_at ? 'bg-slate-100 text-slate-400' : 'bg-brand-100 text-brand-600' }} flex items-center justify-center shrink-0 mt-0.5">
                                        <i
                                    ...
                                    @if(!$notif->read_at)
                                    <span class="w-2 h-2 bg-brand-500 rounded-full mt-2 block shrink-0"></span>
                                    @endif
                                </div>
                                @empty
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button type="button" @click="open = !open"
                            ...
                            <div
                                class="w-7 h-7 rounded-lg bg-brand-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                                {{ substr(Auth::user()->name, 0, 1) }}
                            </div>
                            <div class="text-left hidden sm:block">
                                <p class="text-xs font-semibold text-slate-900 leading-none">{{ Auth::user()->name }}
                                </p>
                                <p class="text-[9px] text-brand-600 font-semibold uppercase tracking-widest mt-0.5">
                                    Admin</p>
                            </div>
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak
                            ...
                            <a href="{{ route('admin.profile') }}"
                                class="flex items-center space-x-2.5 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-brand-600 transition-colors">
                            ...
                            <form id="admin-logout-form" action="{{ route('logout') }}" method="POST" class="hidden">
                                @csrf</form>
                            <button type="button"
                                @click="$dispatch('open-confirm', {type: 'signout', title: 'Sign Out', message: 'Are you sure you want to sign out of the Admin Portal?', formId: 'admin-logout-form'})"
    ...
    <!-- ╔══════════════════════════════════════════╗ -->
    <!-- ║  Notification Detail Modal (Alpine.js)   ║ -->
    <!-- ╚══════════════════════════════════════════╝ -->
            ...
            <div x-show="isOpen" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="px-6 pt-6 pb-4 text-center">
                    <div class="mx-auto mb-4 w-14 h-14 rounded-2xl flex items-center justify-center"
                        :class="isRead ? 'bg-slate-100' : 'bg-brand-50'">
                        <i class="text-2xl hgi-stroke"
                            :class="`${icon} ${isRead ? 'text-slate-400' : 'text-brand-600'}`"></i>
                    </div>
                ...
                <div class="flex">
                    ...
                    <form x-show="!isRead" :action="readUrl" method="POST" class="flex-1">
                        @csrf
                        <button type="submit"
                            class="w-full px-4 py-3.5 text-sm font-bold text-brand-600 hover:bg-brand-50 transition-colors">
                            Mark as Read
                        </button>
                    </form>
                </div>
            ...
            <div x-show="isOpen" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="px-6 pt-6 pb-4 text-center">
                    <div class="mx-auto mb-4 flex items-center justify-center w-14 h-14 rounded-2xl"
                        :class="type === 'delete' ? 'bg-red-50' : (type === 'warning' ? 'bg-amber-50' : 'bg-amber-50')">
                        <i class="text-2xl hgi-stroke"
                            :class="type === 'delete' ? 'hgi-delete-02 text-red-500' : (type === 'warning' ? 'hgi-alert-02 text-amber-500' : 'hgi-logout-01 text-amber-500')"></i>
                    </div>
                    <h3 class="text-base font-bold text-slate-900" x-text="title"></h3>
                ...
                <div class="flex">
                    ...
                    <button @click="confirm()" class="flex-1 px-4 py-3.5 text-sm font-bold transition-colors"
                        ...
                        <span
                            x-text="type === 'delete' ? 'Delete' : (type === 'warning' ? 'Confirm' : 'Sign Out')"></span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div x-data="{ showError: false, errorMsg: '', showSuccess: false }"
        x-init="$nextTick(() => { @if(session('password_changed')) showSuccess = true; @endif })"
        ...
        <div x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95"
            ...
            <!-- Icon Header -->
            <div class="px-6 pt-6 pb-4 text-center">
                <div class="mx-auto mb-4 flex items-center justify-center w-14 h-14 rounded-2xl bg-brand-50">
                    <i class="text-2xl hgi-stroke hgi-lock-password text-brand-600"></i>
                </div>
            ...
            <form method="POST" action="{{ route('password.change') }}" @submit.prevent="
                ...
                <div class="px-6 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" required minlength="8"
                            class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all"
                ...
                <div class="flex">
                    <button type="submit"
                        class="flex-1 px-4 py-3.5 text-sm font-bold text-brand-600 hover:bg-brand-50 transition-colors">
                        Set New Password
                    </button>
                </div>
            </form>
    ...
    <script>
        // Global helper — callable from any onclick without needing an Alpine context
