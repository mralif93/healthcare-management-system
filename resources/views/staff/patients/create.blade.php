    ...
    <div class="bg-indigo-600 rounded-2xl p-8 text-white shadow-xl shadow-indigo-100 relative overflow-hidden animate__animated animate__fadeInUp animate__faster">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
                ...
                <h1 class="text-2xl font-black tracking-tight">New Patient Registration</h1>
                <p class="text-sm text-white/70 mt-1">Identity &amp; bio-data intake</p>
            </div>
            <a href="{{ route('staff.patients.index') }}" class="px-5 py-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white/80 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0">
                Registry
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center space-x-3">
            <div class="w-9 h-9 bg-indigo-50 rounded-xl flex items-center justify-center flex-shrink-0">
                <i class="hgi-stroke hgi-user-add-01 text-indigo-600 text-lg"></i>
            </div>
            <div>
                <h3 class="text-sm font-bold text-slate-900">Patient Identity & Bio-Data</h3>
                <p class="text-[10px] text-slate-400 font-medium mt-0.5">Complete all required fields to register a new patient</p>
            </div>
        </div>
        ...
        <form action="{{ route('staff.patients.store') }}" method="POST">
            ...
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Enter legal full name"
                            class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all shadow-sm">
                    </div>

                    <!-- Email -->
                    <div class="space-y-1.5">
                        <label for="email" class="flex items-center gap-1.5 text-xs font-semibold text-slate-600">
                            <i class="hgi-stroke hgi-mail-01 text-slate-400"></i> Email <span class="text-[10px] text-slate-400 font-normal">(Optional)</span>
                        </label>
                    ...
                    <div class="space-y-1.5">
                        <label for="phone" class="flex items-center gap-1.5 text-xs font-semibold text-slate-600">
                            <i class="hgi-stroke hgi-call text-slate-400"></i> Contact Number <span class="text-red-400">*</span>
                        </label>
                        <input type="text" name="phone" id="phone" value="{{ old('phone') }}" required placeholder="+60 xxx xxx xxxx"
                            class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all shadow-sm">
                    </div>
                ...
                <div class="space-y-1.5">
                    <label for="address" class="flex items-center gap-1.5 text-xs font-semibold text-slate-600">
                        <i class="hgi-stroke hgi-location-01 text-slate-400"></i> Current Address
                    </label>
                    <textarea name="address" id="address" rows="2" placeholder="Street, city, postcode..."
                ...
                <div class="pt-6 border-t border-slate-100">
                    <h4 class="flex items-center gap-2 text-xs font-bold text-slate-700 mb-5">
                        <i class="hgi-stroke hgi-heart-check text-indigo-600"></i> Medical Background
                    </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label for="allergies" class="flex items-center gap-1.5 text-xs font-semibold text-slate-600">
                                <i class="hgi-stroke hgi-alert-circle text-slate-400"></i> Critical Allergies
                            </label>
                            <textarea name="allergies" id="allergies" rows="3" placeholder="Specify any drug or food allergies..."
                                class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all shadow-sm"></textarea>
                        </div>
                        <div class="space-y-1.5">
                            <label for="medical_history" class="flex items-center gap-1.5 text-xs font-semibold text-slate-600">
                                <i class="hgi-stroke hgi-note-01 text-slate-400"></i> Medical History
                            </label>
                            <textarea name="medical_history" id="medical_history" rows="3" placeholder="Chronic illnesses or past operations..."
                                class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all shadow-sm"></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="px-8 py-5 bg-slate-50/70 border-t border-slate-100 flex items-center justify-between">
                <a href="{{ route('staff.patients.index') }}" class="flex items-center space-x-2 px-5 py-2.5 border border-slate-200 text-slate-500 rounded-xl text-sm font-semibold hover:bg-white hover:border-slate-300 transition-all">
