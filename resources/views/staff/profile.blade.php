...
<div class="max-w-4xl mx-auto space-y-6 pb-20">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-8 shadow-sm flex items-center space-x-8">
        <div class="w-24 h-24 bg-brand-50 rounded-2xl flex items-center justify-center text-brand-600 shadow-inner shrink-0">
            <i class="hgi-stroke hgi-user-circle text-5xl"></i>
        </div>
        <div>
            <h2 class="text-2xl font-bold text-slate-900 leading-tight">{{ $user->name }}</h2>
            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">{{ ucfirst($user->role) }} &bull; Clinic Operations</p>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <form action="{{ route('staff.profile.update') }}" method="POST" class="p-8 space-y-8">
            @csrf @method('PUT')
